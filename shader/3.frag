...
const float kernel[KERNEL_SIZE] =
    float[](0.0625, 0.1250, 0.0625, 0.1250, 0.2500, 0.1250, 0.0625, 0.1250, 0.0625);

const vec2 offset[KERNEL_SIZE] = vec2[](vec2(-1.0, -1.0), vec2(0.0, -1.0), vec2(1.0, -1.0),
...
void main()
{
    ...
    if (Set_fxaa == 1) {
        ...
        samplingDirection = clamp(samplingDirection * minSamplingDirectionFactor,
                                  vec2(-u_maxSpan, -u_maxSpan), vec2(u_maxSpan, u_maxSpan)) *
                            u_texelStep;
        ...
        vec3 rgbSampleNegOuter =
            texture(Tex1, TexCoord + samplingDirection * (0.0 / 3.0 - 0.5)).rgb;
        vec3 rgbSamplePosOuter =
            texture(Tex1, TexCoord + samplingDirection * (3.0 / 3.0 - 0.5)).rgb;
