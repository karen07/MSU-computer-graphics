...
const float kernel[KERNEL_SIZE] = float[](0.0625, 0.1250, 0.0625, 0.1250, 0.2500, 0.1250, 0.0625, 0.1250, 0.0625);

const vec2 offset[KERNEL_SIZE] = vec2[](vec2(-1.0, -1.0), vec2(0.0, -1.0), vec2(1.0, -1.0), vec2(-1.0, 0.0), vec2(0.0, 0.0), vec2(1.0, 0.0), vec2(-1.0, 1.0), vec2(0.0, 1.0), vec2(1.0, 1.0));
...
void main() {
	...
	if(Set_dof == 1) {
		...
		vec4 res = vec4(0.0);
		for(int i = 0; i < KERNEL_SIZE; ++i) res += texture(Tex1, TexCoord + offset[i] * pstep) * kernel[i];
